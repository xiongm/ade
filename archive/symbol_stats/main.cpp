...
struct SymbolReport {
  ...
  Price average_price() const {
    return total_price / volumes;
  }
};

class SymbolBook {
public:
  using SymbolMap = std::unordered_map<Symbol, SymbolReport>;
  ...
  void add(Timestamp timestamp, Symbol symbol, Shares shares, Price price) {
    if (!map_.count(symbol)) {
      ...
      map_[symbol] = SymbolReport {symbol, timestamp, 0, shares, shares * price, price};
    } else {
  ...
  friend std::ostream & operator<<(std::ostream &, SymbolBook &);
...
std::ostream & operator<< (std::ostream & os, SymbolBook & book) {
  for (auto p = std::begin(book.ordered_map_); p != std::end(book.ordered_map_); p++) {
    auto & report = book.map_[*p];
    os << report.symbol << ","
       << report.max_time_gap << ","
       << report.volumes << ","
